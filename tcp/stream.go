...
type Stream struct {
	Incoming    <-chan *packet.DataPacket
	Outgoing    chan<- *packet.DataPacket
	in          chan *packet.DataPacket
	out         chan *packet.DataPacket
...
func NewStream(channelBufferSize int) *Stream {
	stream := &Stream{
		in:          make(chan *packet.DataPacket, channelBufferSize),
		out:         make(chan *packet.DataPacket, channelBufferSize),
